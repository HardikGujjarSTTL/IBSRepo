...
namespace IBS.Repositories
{
    public class DownloadBillsRRepository : IDownloadBillsRepository
    {
        ...
        public DownloadBillsModel GetReturnedBills(string Month, string Year, string FromDate, string ToDate, string OrgnType, string Org, string RBMonth)
        {

            DownloadBillsModel model = new();
            List<DownloadBillsModel> lstlab = new();
            string wYrMth;
            ...
            par[0] = new OracleParameter("p_LO_ORGN_TYPE", OracleDbType.NVarchar2, OrgnType, ParameterDirection.Input);
            ...
            par[3] = new OracleParameter("p_fromDt", OracleDbType.NVarchar2, FromDate, ParameterDirection.Input);
            par[4] = new OracleParameter("p_toDt", OracleDbType.NVarchar2, ToDate, ParameterDirection.Input);
            ...
            if (ds.Tables[0].Rows.Count != 0)
            {
                ...
                {
                    string IC = Path.GetFullPath("BILL_IC/" + ds.Tables[0].Rows[0]["IcPhoto"].ToString() + ".PDF");
                    string CaseNo = Path.GetFullPath("CASE_NO/" + ds.Tables[0].Rows[0]["CaseNo"].ToString() + ".PDF");
                    string ICPath = IC;
                    ...
                    {
                        IC = ds.Tables[0].Rows[0]["IcPhoto"].ToString();
                    }
                    ...
                    {
                        CaseNo = ds.Tables[0].Rows[0]["CaseNo"].ToString();
                    }
