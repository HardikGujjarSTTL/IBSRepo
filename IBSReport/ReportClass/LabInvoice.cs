...
namespace IBSReports.ReportClass
{
    public static class LabInvoice
    {
        ...
        public static ReportDocument LabInvoiceReport(string InvoiceNo, string Caseno, string RegNo, string TranNo, out DataSet dsCustom)
        {
            ...
            string str_dt = "Select to_char(INVOICE_DT, 'yyyymm') INVOICE_DT,REGION_CODE REGION_CODE from T55_LAB_INVOICE where INVOICE_NO='"+ InvoiceNo + "'";
            OracleCommand cmd311 = new OracleCommand(str_dt, conn1);
            conn1.Open();
            System.Data.DataSet ds1 = new DataSet();
            OracleDataAdapter adapter1 = new OracleDataAdapter(cmd311);
            adapter1.Fill(ds1);
            int inv_dt = 0;
            string re_cd;
            inv_dt = Convert.ToInt32(ds1.Tables[0].Rows[0][0]);
            re_cd = Convert.ToString(ds1.Tables[0].Rows[0][1]);
            conn1.Close();
        ...
        private static DataSet funlabbill(string InvoiceNo, string Caseno, string RegNo, string TranNo)
        {
            //string conn="";
            ...
            if (Caseno != "")
            {
                sql = sql + " and t55.case_no='" + Caseno + "'";
            }
            if (RegNo != "")
            {

                sql = sql + " and t55.sample_reg_no='" + RegNo + "'";
            }
            if (TranNo != "")
            {

                sql = sql + " and t55.transaction_no='" + TranNo + "'";
            }
