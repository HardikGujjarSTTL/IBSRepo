    ...
    public class IC_RPT : Page
    {
        ...
        private static DataSet funInspectionCertificate_cr(string pcaseno_cr, string pcsno_cr, string recvdt_cr, string pconsidecd_cr, string pregion_cr)
        {
            ...
            string sql_cr = "SELECT     CLR.CASE_NO, CLR.Call_SNO, to_date(TO_CHAR(CLR.Call_Recv_dt, 'mm/dd/yyyy'), 'mm/dd/yyyy')Call_Recv_dt, POM.Region_Code, BOF.BPO_RLY as RLY_CD, CLR.Call_Install_No, TIE.IE_Sname, VND.Vend_Name, VND.Vend_Add1, NVL(VND.Vend_Add2, ' ') AS Vend_Add2,VCT.City AS Vend_City, MFG.Vend_Name AS MFG_Name, MFG.Vend_Add1 AS MFG_Add1,NVL(MFG.Vend_Add2, ' ') AS MFG_Add2, NVL( MCT.City,' ') AS MFG_City, CLR.MFG_PLACE, POM.PO_NO, TO_CHAR(POM.PO_DT,'dd/mm/yyyy') PO_DT, NVL(CON.CONSIGNEE_DESIG, ' ')  AS CONSIGNEE_DESIG, " +
                ...
                " CDT.ITEM_DESC_PO ELSE CDT.ITEM_DESC_PO END END AS ITEM_DESC_PO, UOM.UOM_S_DESC, CASE WHEN CONSGN_CALL_STATUS = 'Z' THEN NVL(ICI.QTY_ORDERED,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN NVL(CDT.QTY_ORDERED,0) ELSE NVL(CDT.QTY_ORDERED,0) END END AS QTY_ORDERED, CASE WHEN (CONSGN_CALL_STATUS = 'Z' OR ICI.CUM_QTY_PREV_OFFERED <> null OR ICI.CUM_QTY_PREV_OFFERED >= 0) THEN NVL(ICI.CUM_QTY_PREV_OFFERED,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN NVL(CDT.CUM_QTY_PREV_OFFERED,0)  ELSE NVL(CDT.CUM_QTY_PREV_OFFERED,0) END END AS CUM_QTY_PREV_OFFERED,  " +
                " CASE WHEN (CONSGN_CALL_STATUS = 'Z' OR ICI.CUM_QTY_PREV_PASSED <> null OR ICI.CUM_QTY_PREV_PASSED >= 0) THEN NVL(ICI.CUM_QTY_PREV_PASSED,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN NVL(CDT.CUM_QTY_PREV_PASSED,0) ELSE NVL(CDT.CUM_QTY_PREV_PASSED,0) END END AS CUM_QTY_PREV_PASSED,  CASE WHEN (CONSGN_CALL_STATUS = 'Z' OR ICI.QTY_TO_INSP <> null OR ICI.QTY_TO_INSP >= 0) THEN NVL(ICI.QTY_TO_INSP,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN NVL(CDT.QTY_TO_INSP,0) ELSE NVL(CDT.QTY_TO_INSP,0) END END AS QTY_TO_INSP, CASE WHEN (CONSGN_CALL_STATUS = 'Z' OR ICI.QTY_PASSED <> null OR ICI.QTY_PASSED >= 0) THEN NVL(ICI.QTY_PASSED,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' " +
                " THEN NVL(CDT.QTY_PASSED,0) ELSE NVL(CDT.QTY_PASSED,0) END END AS QTY_PASSED,   CASE WHEN (CONSGN_CALL_STATUS = 'Z' OR ICI.QTY_REJECTED <> null OR ICI.QTY_REJECTED >= 0) THEN NVL(ICI.QTY_REJECTED,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN NVL(CDT.QTY_REJECTED,0) ELSE NVL(CDT.QTY_REJECTED,0) END END AS QTY_REJECTED,  CASE WHEN (CONSGN_CALL_STATUS = 'Z' OR ICI.QTY_DUE <> null OR ICI.QTY_DUE >= 0) THEN NVL(ICI.QTY_DUE,0) ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN NVL(CDT.QTY_DUE,0) ELSE NVL(CDT.QTY_DUE,0)END END AS QTY_DUE, CASE WHEN CONSGN_CALL_STATUS = 'Z' THEN ICI.HOLOGRAM ELSE CASE WHEN CONSGN_CALL_STATUS = 'A' THEN ICI.HOLOGRAM ELSE ICI.HOLOGRAM " +
